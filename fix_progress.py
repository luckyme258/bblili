...
class BiliPlayer:
    ...
    def clear_list(self):
        ...
        self.entries.clear()
        self.kill_current_mpv()           # 停止播放
        self.current_playing_idx = -1     # 重置索引
        self.current_bvid = None
    ...
    def update_nav_buttons_state(self):
        """根据当前播放索引和分集数量，启用/禁用上/下一集按钮"""
        if not self.entries:
            self.prev_btn.config(state=tk.DISABLED)
            self.next_btn.config(state=tk.DISABLED)
            return
        # 只要 current_playing_idx 在有效范围内，就启用相应按钮（无论 mpv 是否在运行）
        prev_enabled = (self.current_playing_idx > 0)
        next_enabled = (self.current_playing_idx >= 0 and self.current_playing_idx < len(self.entries) - 1)
    ...
    def _load_entries(self, url):
            ...
            def update_listbox():
                ...
                if bvid in progress:
                    ...
                    if 0 <= target_idx < len(self.entries):
                        ...
                        self.current_playing_idx = target_idx
                    else:
        ...
        def update_listbox():
            ...
            self.load_btn.config(state=tk.NORMAL)
            # 尝试高亮上次播放（可能不准确，因为 yt-dlp 未必有页码）
            progress = load_progress()
            if bvid in progress:
                saved_page = progress[bvid]
                target_idx = saved_page - 1
    ...
    def play_by_index(self, idx: int):
        ...
        if self.current_bvid:
            # 从 URL 中提取 p 参数，如果没有则从 entries 的索引推断页码
            p_match = re.search(r'[?&]p=(\d+)', url)
            if p_match:
                page = int(p_match.group(1))
            else:
                # 如果没有 p 参数，则使用索引+1作为页码（适用于 API 构建的链接）
                page = idx + 1
