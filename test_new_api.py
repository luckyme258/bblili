...
# ========== 🔥 使用 bilibili-api-python 获取分P列表 ==========
def get_video_pages_from_api(bvid: str) -> Optional[List[Tuple[str, int, str]]]:
    """使用第三方库获取视频分 P 信息（自动处理 WBI 签名）"""
    try:
        debug_print(f"使用 bilibili-api 获取视频信息: {bvid}")
        v = video.Video(bvid=bvid)
        info = sync(v.get_info())  # 同步方式调用异步API，自动处理签名
        ...
        debug_print(f"bilibili-api 请求异常: {e}")
        # 可选：降级到原来的 requests 方案
        return fallback_to_old_api(bvid)

def fallback_to_old_api(bvid: str) -> Optional[List[Tuple[str, int, str]]]:
    """备用方案：使用原生 requests（保持兼容性）"""
    try:
        import httpx
        ...
        data = resp.json()
        if data['code'] != 0:
            debug_print(f"备用 API 返回错误: code={data['code']}")
            return None
        ...
        for page in pages:
            ...
            play_url = f"https://www.bilibili.com/video/{bvid}?p={page_num}"
            result.append((title, page_num, play_url))
        debug_print(f"备用 API 获取成功，共 {len(result)} 集")
        return result
    except Exception as e:
        debug_print(f"备用 API 也失败了: {e}")
        return None
...
class BiliPlayer:
    def __init__(self, root):
        self.root = root
        self.root.title("B站播放器 - 网格列表 + 编码显示 (bilibili-api版)")
    ...
    def _load_entries(self, url):
        ...
        update_status(f"正在请求 API (bilibili-api): {bvid}")
    ...
    def play_episode_by_index(self, idx: int):
        ...
        def play_thread():
            def status_cb(msg):
                self.root.after(0, lambda: self.status_var.set(msg))
            err, proc, codec_name = start_mpv_process(url, self.current_strategy, status_cb)
            if err:
